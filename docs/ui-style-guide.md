...
| brand | `bg-brand`, `hover:bg-brand-hover`, `bg-brand-soft`, `text-brand-ink` | Акцент `#1d83c4` |
...
| accent | `bg-accent-soft`, `text-accent-ink` | тег `ai` |
| elev | `rounded-card` / `rounded-card-lg`, `shadow-card` | Карточки 20–24px |

Оверлей модалок: `var(--overlay)` / класс `.modal-overlay`. Сайдбар: `var(--sidebar)` (`#eef7ff`).
...
| `.badge-*` | Статусы и теги (`info`/`accent`/`success`/`warning`/`danger`/`neutral`) |
