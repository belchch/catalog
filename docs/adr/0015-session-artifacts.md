...
- **Revised by:** ADR-0021 (при выбранном треке build уходит на LLM-путь мимо упаковки)
...
## Уточнение (2026-07-24): когда build всё-таки зовёт LLM

Пункт 4 («build = упаковка») описывает **основной** путь. В коде
(`backend/app/api/skills.py:654`) есть два выхода из него:

1. **артефактов нет вовсе** — legacy LLM-fallback (как и описано выше);
2. **в истории есть выбранный трек операции** (сообщение с префиксом
   «Собери скилл по этой операции:») — `force_llm`: упаковка **пропускается**
   целиком, потому что артефакты писались планировщиком до выбора операции и
   её не учитывают (ADR-0021).

Следствие, о котором стоит помнить: на втором пути ручные правки
`prompt`/`script` в панели артефактов в собранный скилл не попадут, а build
снова стоит LLM-вызова и снова подвержен таймауту — ровно тому, от чего
уходил этот ADR.
