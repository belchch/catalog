...
- **Тип шага:** code
- **Цель:** Упаковать backend как устанавливаемое приложение: `uv tool install` → команда `catalog` поднимает uvicorn на 127.0.0.1, отдаёт собранный фронт, читает ключи не из `.env` в CWD. UI экрана ключа — парный план `CATALOG-83-ui-first-run-api-key.md`.
...
Отдельный этап после воркспейсов (77–80). Парный UI: `CATALOG-83-ui-first-run-api-key.md` (после этого code-шага).
