...
(``[run=… iter=… purpose=… session=…]``) read from :mod:`app.llm.log_context`
...
class AppFormatter(_UvicornDefaultFormatter):
    ...
    prefix, no timestamp, no logger name) so ``app.*`` lines line up with native
...
def setup_logging(level: str | None = None) -> None:
    ...
    ``level`` overrides the ``app`` logger level (defaults to INFO when
