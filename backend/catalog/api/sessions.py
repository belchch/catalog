...
@router.post("/sessions/{session_id}/tools", response_model=list[SkillOut])
async def attach_session_tools_endpoint(
    ...
    if get_session(db, session_id) is None:
        raise HTTPException(status_code=404, detail="session not found")
    raw_ids = body.get("skill_ids") if isinstance(body, dict) else None
    if not isinstance(raw_ids, list) or not raw_ids:
        raise HTTPException(status_code=422, detail="skill_ids must be a non-empty list")
    skill_ids = [str(x) for x in raw_ids if x]
    skipped = attach_skills(db, session_id, skill_ids)
    if skipped and len(skipped) == len(skill_ids):
        raise HTTPException(status_code=404, detail=f"skills not found: {skipped}")
    return [_session_skill_out(s) for s in list_session_skills(db, session_id)]
