...
export function SessionsPanel({
  ...
  return (
    <div className="flex flex-col gap-2">
      {sessions.error && <p className="text-xs text-danger-ink">{sessions.error}</p>}
      ...
      {!sessions.loading && sessions.sessions.length === 0 && !sessions.error && (
        <p className="text-xs text-ink-faint">Пока нет сохранённых сессий</p>
      )}
              ...
              <div className="flex shrink-0 flex-col justify-center gap-0.5">
                {confirming ? (
                  <>
                    <button
                      type="button"
                      className="rounded px-1.5 py-0.5 text-[10px] text-danger-ink hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
                      onClick={(e) => {
                        e.stopPropagation()
                        setConfirmId(null)
                        onDelete(s.id)
                    ...
                    <button
                      type="button"
                      className="rounded px-1.5 py-0.5 text-[10px] text-ink-faint hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
                      onClick={(e) => {
                        e.stopPropagation()
                        clearConfirm()
                      }}
                  ...
                  <button
                    type="button"
                    className="rounded px-1.5 py-1 text-xs text-ink-faint hover:text-danger-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
