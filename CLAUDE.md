...
В пайплайне проверки гоняют **две** роли: `catalog-generator` (рабочий прогон,
отчёт уходит ревьюеру как CHECKS) и `catalog-steward` в `finalize`
(авторитетный прогон перед коммитом). Ревьюеры проверки **не дублируют** —
третий прогон за цикл был измеренной потерей ~20 минут на смену.
